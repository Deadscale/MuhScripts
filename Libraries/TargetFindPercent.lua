...
function targetFind:GetHighestPercentTBlade(range,dmg_type)
	local me = entityList:GetMyHero()
	local enemyTeam = me:GetEnemyTeam()
	...
	for _,v in ipairs(enemies) do
		if me:GetDistance2D(v) < range then
			...
			local distance = GetDistance2D(me,v)
			if distance <= range and v.alive and v.visible then 
				if not result or (result.health*l_multipler)/result.maxHealth < (v.health*v_multipler)/v.maxHealth then
					result = v
				end
			end
		end
...
function targetFind:GetHighestPercentHuskar(range,dmg_type)
	local me = entityList:GetMyHero()
	local enemyTeam = me:GetEnemyTeam()
	...
	for _,v in ipairs(enemies) do
		if me:GetDistance2D(v) < range then
			...
			local distance = GetDistance2D(me,v)
			if distance <= range and v.alive and not v:IsIllusion() and v.visible and not v:IsMagicImmune() then 
				if not result or (result.health*l_multipler)/result.maxHealth < (v.health*v_multipler)/v.maxHealth then
					result = v
				end
			end
